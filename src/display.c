...
/*---------------------------------------------------*/
/* Display text variables for LCD */
char deviceName[DISPLAY_ROW_LEN];
connectionState_t currConnState = CONNECTION_ADVERTISING;
char deviceConnState[DISPLAY_ROW_LEN];
uint8_t pDeviceBtAddr[6];
uint32_t passkeyValue = 0;
...
#warning "gpioEnableDisplay is not implemented, please implement in order to use the display"
  ...
  /* Start timer to trigger at 1Hz for LCD display updates */
...
{
  currConnState = connState;

  switch(connState) {
    case(CONNECTION_ADVERTISING):
      strcpy(deviceConnState, "Advertising");
      break;
    case(CONNECTION_DISCOVERING):
      strcpy(deviceConnState, "Discovering");
      break;
    case(CONNECTION_ESTABLISHED):
      strcpy(deviceConnState, "Connected");
      break;
    case(CONNECTION_LOST):
      strcpy(deviceConnState, "Connection Lost");
      break;
    case(CONNECTION_ADV_TIMEOUT):
      strcpy(deviceConnState, "Advertising Timeout");
      break;
    case(HANDLE_INDICATIONS):
      strcpy(deviceConnState, "Handling Indications");
      break;
    case(CONNECTION_BONDING):
      strcpy(deviceConnState, "Conn Bonding");
      break;
    case(CONNECTION_BONDED):
      strcpy(deviceConnState, "Conn Bonded");
      break;
    case(CONNECTION_BONDING_FAILED):
      strcpy(deviceConnState, "Conn Bond Failed");
      break;
    default:
      break;
  }
...
void displayTextToLcd()
{
  ...
  displayPrintf(DISPLAY_ROW_NAME, deviceName);
  displayPrintf(DISPLAY_ROW_CONNECTION, deviceConnState);

  if(currConnState == CONNECTION_BONDING){
    /* Convert uint32 to string and display to LCD */
    char passkey[9];
    sprintf(passkey, "%d", passkeyValue);
    displayPrintf(DISPLAY_ROW_PASSKEY, passkey);
    displayPrintf(DISPLAY_ROW_ACTION, "Confirm with PB0");
  } else if((currConnState != CONNECTION_BONDING) && (passkeyValue != 0)) {
    /* Clear Passkey fields if not in Bonding state */
  ...
  displayPrintf(DISPLAY_ROW_TIME, "Time: %s", timeStr);

  /* Display Last Motion Detect Event */
  displayPrintf(DISPLAY_ROW_LAST_MOTION_EVT, "Last Evt: %s", lastMotionEventTime.timeStr);

  /* Display Motion Detect schedule */
  displayPrintf(DISPLAY_ROW_DETECT_SCHED, "Sched (Hr): %s", configSchedStr);
}
/*---------------------------------------------------*/
